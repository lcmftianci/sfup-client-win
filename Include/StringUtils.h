...
Returns a_Dst. */
COMMON_API AString & AppendVPrintf(AString & a_Dst, const char * format, va_list args) FORMATSTRING(2, 0);
...
COMMON_API AString Printf(const char * format, ...) FORMATSTRING(1, 2);

/** Add the formatted string to the existing data in the string.
Returns a_Dst */
COMMON_API AString & AppendPrintf (AString & a_Dst, const char * format, ...) FORMATSTRING(2, 3);

/** Split the string at any of the listed delimiters.
Return the splitted strings as a stringvector. */
COMMON_API AStringVector StringSplit(const AString & str, const AString & delim);

/** Split the string at any of the listed delimiters. Keeps quoted content together
Resolves issue #490
Return the splitted strings as a stringvector. */
COMMON_API AStringVector StringSplitWithQuotes(const AString & str, const AString & delim);

/** Split the string at any of the listed delimiters and trim each value.
Returns the splitted strings as a stringvector. */
COMMON_API AStringVector StringSplitAndTrim(const AString & str, const AString & delim);

COMMON_API AStringVector SplitOneOf(const AString& str, const AString& delims, const size_t maxSplits = 0);

/** Trims whitespace at both ends of the string.
Returns a trimmed copy of the original string. */
COMMON_API AString TrimString(const AString & str);  // tolua_export

/** In-place string conversion to uppercase.
Returns the same string object. */
COMMON_API AString & InPlaceUppercase(AString & s);

/** In-place string conversion to lowercase.
Returns the same string object. */
COMMON_API AString & InPlaceLowercase(AString & s);

/** Returns an upper-cased copy of the string */
COMMON_API AString StrToUpper(const AString & s);

/** Returns a lower-cased copy of the string */
COMMON_API AString StrToLower(const AString & s);

/** Case-insensitive string comparison.
Returns 0 if the strings are the same, <0 if s1 < s2 and >0 if s1 > s2. */
COMMON_API int NoCaseCompare(const AString & s1, const AString & s2);  // tolua_export

/** Case-insensitive string comparison that returns a rating of equal-ness between [0 - s1.length()]. */
COMMON_API size_t RateCompareString(const AString & s1, const AString & s2);

/** Replaces each occurence of iNeedle in iHayStack with iReplaceWith */
COMMON_API void ReplaceString(AString & iHayStack, const AString & iNeedle, const AString & iReplaceWith);  // tolua_export

/** Converts a stream of BE shorts into UTF-8 string; returns a_UTF8. */
COMMON_API AString & RawBEToUTF8(const char * a_RawData, size_t a_NumShorts, AString & a_UTF8);

/** Converts a UTF-8 string into a UTF-16 BE string. */
COMMON_API std::u16string UTF8ToRawBEUTF16(const AString & a_String);

/** Creates a nicely formatted HEX dump of the given memory block.
Max a_BytesPerLine is 120. */
COMMON_API AString & CreateHexDump(AString & a_Out, const void * a_Data, size_t a_Size, size_t a_BytesPerLine);

/** Returns a copy of a_Message with all quotes and backslashes escaped by a backslash. */
COMMON_API AString EscapeString(const AString & a_Message);  // tolua_export

/** Removes all control codes used by MC for colors and styles. */
COMMON_API AString StripColorCodes(const AString & a_Message);  // tolua_export

/** URL-Decodes the given string, replacing all "%HH" into the correct characters. Invalid % sequences are left intact */
COMMON_API AString URLDecode(const AString & a_String);  // Cannot export to Lua automatically - would generated an extra return value

/** Replaces all occurrences of char a_From inside a_String with char a_To. */
COMMON_API AString ReplaceAllCharOccurrences(const AString & a_String, char a_From, char a_To);  // Needn't export to Lua, since Lua doesn't have chars anyway

/** Decodes a Base64-encoded string into the raw data */
COMMON_API AString Base64Decode(const AString & a_Base64String);  // Exported manually due to embedded NULs and extra parameter

/** Encodes a string into Base64 */
COMMON_API AString Base64Encode(const AString & a_Input);  // Exported manually due to embedded NULs and extra parameter

/** Reads two bytes from the specified memory location and interprets them as BigEndian short */
COMMON_API short GetBEShort(const char * a_Mem);

/** Reads four bytes from the specified memory location and interprets them as BigEndian int */
COMMON_API int GetBEInt(const char * a_Mem);

/** Writes four bytes to the specified memory location so that they interpret as BigEndian int */
COMMON_API void SetBEInt(char * a_Mem, Int32 a_Value);
...
COMMON_API bool SplitZeroTerminatedStrings(const AString & a_Strings, AStringVector & a_Output);
...
COMMON_API AStringVector MergeStringVectors(const AStringVector & a_Strings1, const AStringVector & a_Strings2);

/** Concatenates the specified strings into a single string, separated by the specified separator. */
COMMON_API AString StringsConcat(const AStringVector & a_Strings, char a_Separator);
