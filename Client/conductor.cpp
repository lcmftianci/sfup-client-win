...
Conductor::Conductor(ConnectionClient* client, MainWindow* main_wnd): m_client(client),m_main_wnd(main_wnd) 
{
	m_client->RegisterObserver(this);
	m_main_wnd->RegisterObserver(this);

	if (!Initialize())
	{
		m_main_wnd->MessageBox("Error", "Failed to initialize PeerConnection", true);
	}

	m_client->Init(SERVER, PORT, GetPeerName());
}

Conductor::Conductor(telemetry_client* webclient, MainWindow* main_wnd) : m_webclient(webclient), m_main_wnd(main_wnd)
{
	m_webclient->RegisterObserver(this);
	m_main_wnd->RegisterObserver(this);
...
//************************************
// Method:    SendMessage
// FullName:  Conductor::SendMessage
// Access:    virtual public 
// Returns:   void
// Qualifier:
// Parameter: UInt32 id
// Parameter: const AString & json_object
//************************************
void Conductor::SendMessage(UInt32 id, const AString& json_object) {
	...
	m_main_wnd->QueueUIThreadCallback(SEND_MESSAGE_TO_PEER, msg);
}

//************************************
// Method:    QueueUIThreadCallback
// FullName:  Conductor::QueueUIThreadCallback
// Access:    virtual public 
// Returns:   void
// Qualifier:
...
{
	m_main_wnd->QueueUIThreadCallback(msg_id, data);
}

//************************************
// Method:    SetLocalDescription
// FullName:  Conductor::SetLocalDescription
// Access:    virtual public 
// Returns:   void
// Qualifier:
// Parameter: UInt32 id
// Parameter: webrtc::SessionDescriptionInterface * desc
//************************************
...
bool Conductor::CreatePeerConnection(UInt32 id, bool dtls) 
{
	...
	server.uri = "stun:39.106.13.209:3478";
	server.urls.push_back("turn:39.106.13.209:3478");
	server.urls.push_back("turn:39.106.13.209:3478?transport=udp");
	server.urls.push_back("turn:39.106.13.209:3478?transport=tcp");
	server.username = "along";
	server.password = "apassword";
	servers.push_back(server);

	//webrtc::PeerConnectionInterface::IceServer server1;
	//server1.uri = "turn:39.106.13.209:3478";
	//server1.username = "along";
	//server1.password = "apassword";
	...
	rtc::scoped_refptr<webrtc::PeerConnectionInterface> peer_connection =
		peer_connection_factory_->CreatePeerConnection(servers,	&constraints,NULL,NULL,	pcObserver);
...
void Conductor::AddStreams() 
{
	...
	cricket::VideoCapturer *video = OpenDesktopCaptureDevice();
	if (video)
	{
		webrtc::FakeConstraints constraints;
...
void Conductor::OnMessageFromChannel(const AString &room, const UInt32 &a_FromID, const AString& a_Msg)
{
	...
	LOG(WARNING) << "OnMessageFromChannel: " << "was: " << a_Msg;
	...
	else if (type == "offer" || type == "answer") 
	{
		...
		{
			LOG(WARNING) << "Can't parse received session description a_Msg. "	<< "SdpParseError was: " << error.description;
			return;
		}
